...
export function HowItWorks() {
  return (
    <section
      id="process"
      className="content-section scroll-mt-28 px-5 py-10 sm:px-8 sm:py-16 lg:px-10"
    >
      <div className="reveal hover-lift mx-auto max-w-7xl rounded-[2.5rem] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-soft)] sm:p-8 lg:p-10">
        ...
        <ol className="mt-6 grid gap-3 sm:mt-8 sm:gap-4 lg:mt-10 lg:grid-cols-4">
          {steps.map((step, index) => (
            <li
              key={step}
              className="hover-lift relative flex items-start gap-4 rounded-[1.25rem] border border-[var(--border)] bg-[rgba(247,241,232,0.58)] p-4 sm:block sm:rounded-[1.75rem] sm:p-6"
            >
              <span
                aria-hidden="true"
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[var(--border)] bg-[var(--surface)] text-sm font-medium text-[var(--accent)] sm:hidden"
              >
                {index + 1}
              </span>
              ...
              <p className="text-lg font-medium leading-snug tracking-[-0.03em] text-[var(--text)] sm:mt-8 sm:text-xl">
                {step}
              </p>
            </li>
          ))}
        ...
        <div className="mt-6 rounded-[1.5rem] border border-[var(--border)] bg-[rgba(255,251,245,0.76)] p-4 text-sm leading-6 text-[var(--muted)] sm:mt-8 sm:p-5 sm:text-base sm:leading-7">
